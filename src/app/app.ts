...
export class App {
  ...
  chatMessages = signal<ChatMessage[]>([
    {
      id: '1',
      content: 'Hello! I\'m your AI assistant. I can help you with your notes, answer questions, and provide insights.',
      sender: 'ai',
      timestamp: new Date()
    }
  ]);
  ...
  // Markdown preview computed property
  ...
  renderLineMarkdown(line: string): SafeHtml {
    if (!line.trim()) {
      return this.sanitizer.bypassSecurityTrustHtml('<div class="empty-line">&nbsp;</div>');
    }
  ...
  shouldRenderLine = computed(() => {
    ...
    return (lineIndex: number): boolean => {
      ...
      if (editingIndex === lineIndex || focusedIndex === lineIndex) return false;
  ...
  selectNote(note: Note) {
    ...
    // Reset line editing states when switching notes
    this.editingLineIndex.set(null);
    this.lineRenderModes.set({});
  }
  ...
  updateNote() {
    ...
    selected.preview = contentPreview.substring(0, 50) + (contentPreview.length > 50 ? '...' : '');
    selected.lastModified = new Date();
    
    // Update the note in the array
    this.notes.update(notes => 
      notes.map(note => note.id === selected.id ? { ...selected } : note)
    );
  }
  ...
  private checkForBacklinkTyping(content: string, lineIndex: number) {
    ...
    if (backlinkMatch) {
      ...
      const suggestions = this.notes().filter(note => 
        note.title.toLowerCase().includes(query) && 
  ...
  insertBacklinkSuggestion(suggestion: Note) {
    ...
    const updatedLine = currentLine.replace(/\[\[([^\]]*?)$/, `[[${suggestion.title}]]`);
  ...
  handleLineKeydown(event: KeyboardEvent, lineIndex: number) {
    ...
    switch (event.key) {
      ...
      case 'ArrowRight':
        ...
        if (currentSelected && cursorPos >= (target.textContent?.length || 0) && lineIndex < currentSelected.lines.length - 1) {
          event.preventDefault();
  ...
  focusLine(lineIndex: number, cursorPos: number = 0) {
    setTimeout(() => {
      const lineElement = document.querySelector(`[data-line-index="${lineIndex}"]`) as HTMLElement;
  ...
  handleRenderedLineClick(lineIndex: number, event?: MouseEvent) {
    ...
    if (event && (event.target as HTMLElement).classList.contains('backlink')) {
      const noteTitle = (event.target as HTMLElement).getAttribute('data-note-title');
      if (noteTitle) {
  ...
  sendMessage(event?: KeyboardEvent | Event) {
